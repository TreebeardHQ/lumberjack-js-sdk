...
describe('Source Map Upload', () => {
  ...
  describe('findSourceMapFiles', () => {
    it('should find both client-side and server-side source maps', async () => {
      ...
      expect(result).toHaveLength(clientSourceMaps.length + serverSourceMaps.length);

      // Check client-side source maps have correct URL patterns
      const clientResults = result.filter(file => file.url.includes('/_next/static/'));
      ...
      const serverResults = result.filter(file => file.url.includes('/_next/server/'));
      ...
      expect(result.some(f => f.url.includes('/api/hello/route.js.map'))).toBe(true);
      expect(result.some(f => f.url.includes('/app/page.js.map'))).toBe(true);
      expect(result.some(f => f.url.includes('/chunks/main.js.map'))).toBe(true);
    });
    ...
    it('should include API route source maps', async () => {
      ...
      const apiRouteMap = join(serverDir, 'app', 'api', 'hello', 'route.js.map');
      writeFileSync(apiRouteMap, JSON.stringify({
        version: 3,
        sources: ['route.ts'],
        names: [],
        mappings: 'AAAA',
      ...
      expect(result[0].url).toBe('~/_next/server/app/api/hello/route.js.map');
    ...
    it('should include server component source maps', async () => {
      ...
      writeFileSync(pageMap, JSON.stringify({
        version: 3,
        sources: ['page.tsx'],
        names: [],
        mappings: 'AAAA',
      ...
      expect(result).toHaveLength(1);
      expect(result[0].url).toBe('~/_next/server/app/page.js.map');
  ...
  describe('integration', () => {
    it('should upload both client and server source maps', async () => {
      ...
      writeFileSync(join(serverDir, 'app', 'api', 'test', 'route.js.map'), '{"version":3}');

      const { findSourceMapFiles, createSourceMapFormData } = await import('./config');
