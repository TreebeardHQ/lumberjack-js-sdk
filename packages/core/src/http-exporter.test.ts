...
describe('HttpExporter', () => {
  ...
  describe('exportLogs', () => {
    ...
    it('should handle HTTP errors', async () => {
      ...
      expect(result.error?.message).toContain('Failed to send logs: 500 Internal Server Error - Server error details');
      expect(result.itemsExported).toBe(0);
    });
  ...
  describe('exportObjects', () => {
    ...
    it('should handle HTTP errors', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error?.message).toContain('Failed to send objects: 401 Unauthorized - Invalid API key');
      expect(result.itemsExported).toBe(0);
    });
